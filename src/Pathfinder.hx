...
class Pathfinder {
	...
	public function new(worldSquare:WorldSquare) {
		...
		a_pathfinder = new AStarPathfinder(map);
	}

	public function generatePath(startX:Int, startY:Int, destX:Int, destY:Int) {
...
interface SimpleMap {
	/**
	 * Total rows (horizontal divisions) in the Map (y)
	 */
	public var rows(default, never):Int;

	/**
	 * Total columns (vertical divisions) in the Map (x)
	 */
	public var cols(default, never):Int;

	/**
	 * Returns whether a coordinate on the map is walkable, or obstructed.
	 * @param	p_x	The column
	 * @param	p_y	The row
	 * @return	true if walkable, false if obstructed
	 */
	public function isWalkable(p_x:Int, p_y:Int):Bool;
}

class MapData implements SimpleMap {
	public var rows(default, null):Int;
	public var cols(default, null):Int;

	private var worldSquare:WorldSquare;

	public function new(worldSquare:WorldSquare, p_cols:Int, p_rows:Int) {
	...
	public function isWalkable(p_x:Int, p_y:Int):Bool {
		return worldSquare.checkNavMesh(p_x, p_y);
	}
}
...
class AStarPathfinder {
	...
	private var _info:{
		...
		timeElapsed:Int,
		pathLength:Int,
		isDiagonalEnabled:Bool
	};

	/**
	 * Creates a new AStarPathfinder class
	 * @param	p_map	The boolean coordinate map
	 * @param	p_timeOutDuration	The maximum time spent to find a path
	 */
	public function new(p_map:SimpleMap, p_timeOutDuration:Int = 10000) {
		configure(p_map, p_timeOutDuration);
	}

	/**
	 * Reconfigures an existing AStarPathfinder class
	 * @param	p_map	The boolean coordinate map
	 * @param	p_timeOutDuration	The maximum time spent to find a path
	 */
	public function configure(p_map:SimpleMap, p_timeOutDuration:Int = 10000) {
		_map = p_map;
	 ...
	 * @param	p_isMapDynamic	Set to true to force fresh lookups from SimpleMap.isWalkable() for each node's isWalkable property (e.g. for a dynamically changing map)
	 * @return	An array of coordinates from start to destination, or null if no path was found within the time limit
	 */
	public function createPath(p_start:Coordinate, p_dest:Coordinate, ?p_heuristic:EHeuristic, p_isDiagonalEnabled:Bool = true,
			p_isMapDynamic:Bool = false):Array<Coordinate> {
		if (p_heuristic == null) {
	...
	public function getInfo():String {
		return _isCompleted ? "Success using "
			+ _info.heuristic
			+ (!_info.isDiagonalEnabled ? " (and diagonals disabled )" : "")
